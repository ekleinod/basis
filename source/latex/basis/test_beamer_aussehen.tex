% very ugly hack to suppress frames in articles
\newcounter{basslide}
\setcounter{basslide}{1}

\let\origframe=\frame
\let\origendframe=\endframe

\mode<presentation>{
	\def\frame{%
		\origframe%
		\label{basslide\roman{basslide}}%
		\addtocounter{basslide}{1}%
	}
	\def\endframe{\origendframe}
}

\mode<article>{
	\usepackage{verbatim} % Auskommentieren des Frame-Inhalts
	\def\frame{%
		\noindent%
		{%
			\par \centering%
			\includeslide[width=.8\textwidth]{basslide\roman{basslide}}%
			\par%
		}%
		\addtocounter{basslide}{1}%
		\comment%
...
\usetheme[titleprogressbar, totalnumber]{basis}
