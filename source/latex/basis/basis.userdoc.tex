...
Mögliche Optionen: \option{bewerbung}, \option{draft}, \option{fixme}, \option{font}, \option{hypercolor}, \option{hyperdriver}, \option{index}, \option{layout}, \option{nohyper}, \option{nojura}, \option{noonelinecaption} und \option{onehalfspacing}-Option.
...
Die \option{fixme}-Option sorgt für die Einbindung des \paket{fixme}-Pakets und definiert dessen Layout sinnvoll.
Der Option können die Werte \option{draft} und \option{final} übergeben werden, die an das Paket weitergeleitet werden.
Wird nichts angegeben, gilt die Option \option{final} als gewählt.

\verb#\usepackage[fixme]{basis}#\\
\verb#\usepackage[fixme=draft]{basis}#

Benutzbare \paket{fixme}-Befehle: \verb#\fixme#, \verb#\fxnote#, \verb#\fxwarning#, \verb#\fxerror#
...
Die \option{hyperdriver}-Option stellt den Treiber des \paket{hyperref}-Pakets ein statt \verb#ps2pdf#.

\verb#\usepackage[hyperdriver=pdftex]{fhg_article}#

\minisec{index}

Die \option{index}-Option bewirkt, dass Befehle zur Indexierung vorbereitet
werden und die \verb#makeindex#-Umgebung mit Hilfe des Pakets \paket{makeidx}
geladen wird.
...
Die Option \option{nohyper} verhindert das Einbinden des \paket{hyperref}-Pakets.

\verb#\usepackage[nohyper]{basis}#

\minisec{nojura}

Die Option \option{nojura} verhindert das Einbinden des \paket{jurabib}-Pakets.

\verb#\usepackage[nojura]{basis}#

\minisec{noonelinecaption}

Die \option{noonelinecaption}-Option ist aus dem Koma-Script-Paket entlehnt.
Sie braucht nicht extra angegeben werden, da sie von den globalen Optionen übernommen wird und redefiniert die Tabellenüberschriften von \paket{longtable}-Tabellen.
Sie wirkt nur bei Angabe der \option{origlongtable}-Option.

\verb#\documentclass[noonelinecaption, origlongtable]{scrartcl}#\\
\verb#\usepackage{basis}#

\minisec{onehalfspacing}

Die Option \option{onehalfspacing} setzt Text anderthalbzeilig.

\verb#\usepackage[onehalfspacing]{basis}#
