...
\begin{tabbing}
	...
	\paket{siunitx}\>SI-Einheiten setzen\\
	\paket{tabu}\>Tabellenerweiterung\\
	\paket{wasysym}\>Sonderzeichen (wenn Paket fehlt, wird eine Warnung ausgegeben)\\
	\paket{xcolor}\>Textfärbung (mit Optionen \option{svgnames} und \option{table})\\
...
Mögliche Optionen: \option{draft}, \option{final}, \option{font}, \option{fontsize}, \option{hypercolor}, \option{hyperdriver}, \option{layout}, \option{noauthor}, \option{nobackaddress}, \option{nofoldmarks}, \option{nosponsorlogo}, \option{notitlepage}, \option{oneside}, \option{pagestyle}, \option{protocol}, \option{rules}
...
Bei der Einbindung von \paket{xcolor} werden die Optionen \option{svgnames} und \option{table} gesetzt, so dass mehr Farbnamen zur Verfügung stehen und das Paket \paket{colortbl} geladen wird, mit dem Tabellen eingefärbt werden können.
...
Die \option{nosponsorlogo}-Option unterdrückt die Ausgabe des Sponsorlogos.

\begin{nutzung}
		\>\befehl{usepackage[nosponsorlogo]\{basis\}}\\
\end{nutzung}
...
Die Option \option{orientation} setzt die Seitenorientierung: hoch- oder querkant.

Standard: \option{portrait}

Andere Werte: \option{landscape}, \option{seascape}

\begin{nutzung}
		\>\befehl{usepackage[orientation=\meta{Orientierung}]\{basis\}}\\
	\beispiel
		\>\befehl{usepackage[orientation=landscape]\{basis\}}
\end{nutzung}
...
Die Option \option{paper} setzt die Seitengröße.

Standard: \option{a4}

\begin{nutzung}
		\>\befehl{usepackage[paper=\meta{Seitengröße}]\{basis\}}\\
	\beispiel
		\>\befehl{usepackage[paper=a5]\{basis\}}
\end{nutzung}
...
\subsubsection{tabulary}

Die \option{tabulary}-Option lädt das \paket{tabulary}-Paket.
Das Paket wird von Multimarkdown standardmäßig für das Setzen von Tabellen genutzt.

\begin{nutzung}
		\>\befehl{usepackage[tabulary]\{basis\}}\\
\end{nutzung}
